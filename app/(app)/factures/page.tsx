import { Suspense } from "react";
import { exigerCompte } from "@/lib/auth/compte";
import { lireReglages } from "@/lib/reglages/requetes";
import { anneeDemandee } from "@/lib/facture/annee";
import { lireAnnees, lireFactures } from "@/lib/facture/requetes";
import { SelecteurAnnee } from "@/modules/finance/SelecteurAnnee";
import { Factures } from "@/modules/finance/Factures";

export default async function PageFactures({
  searchParams,
}: {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}) {
  await exigerCompte();

  const annee = anneeDemandee((await searchParams).annee);

  const [reglages, factures, annees] = await Promise.all([
    lireReglages(),
    lireFactures(annee),
    lireAnnees(),
  ]);

  return (
    <>
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl">Tes factures</h1>
          <p className="mt-2 text-sm text-texte-doux">
            Une facture émise, et le jour où elle a été payée.
          </p>
        </div>
        <Suspense fallback={null}>
          <SelecteurAnnee annee={annee} annees={annees} />
        </Suspense>
      </div>

      <div className="mt-8">
        <Factures factures={factures} devise={reglages.devise} annee={annee} />
      </div>
    </>
  );
}
